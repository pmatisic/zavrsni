...
class HandDetector :
    def __init__(self ,mode=False , maxHand=2 , detectCon=0.5 ,trackCon=0.5):
        ...
        # Sadrži način otkrivanja i način praćenja. Kada nemamo dostupnih informacija za praćenje, tada prelazi u način otkrivanja.
        ...
        # Ova biblioteka (ruka) samo uzima RGB sliku.
    ...
    def findhands(self, img, draw = True):
        ...
        if self.processHand.multi_hand_landmarks:  # Može se koristiti processHand.multi_hand_landmarks[0] ili [1].
        ...
        if self.processHand.multi_hand_landmarks:  # Može se koristiti processHand.multi_hand_landmarks[0] ili [1].
            try:
                ...
                for id, lm in enumerate(myHand.landmark):
                    #print(id, lm)  
                    # Ruke sadrže 21 točku [0 do 20] i svaka točka označava neki dio ruke.
                    # Problem je što je 'lm' u decimalnom obliku i trebamo piksel npr.:(200 w, 300 h).
                    h, w, c = img.shape
                    cx, cy = int(lm.x * w), int(lm.y * h)  # Sada imamo oblik (id, cx, cy), pa možemo učiniti bilo što.
